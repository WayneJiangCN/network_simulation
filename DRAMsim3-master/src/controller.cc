...
bool Controller::WillAcceptTransaction(uint64_t hex_addr, bool is_write) const {
    //std::cout<<"hex_addr:"<<hex_addr<<std::endl;
    if (is_unified_queue_) {
        // if(channel_id_==7){
   ...
   // std::cout<<"read_queue_.capacity()"<<read_queue_.capacity()<<std::endl;
        // if(channel_id_==7){
        // std::cout<<"read.size"<<read_queue_.size()<<std::endl;
        // }
        return read_queue_.size() < read_queue_.capacity();
    } else {
        // if(channel_id_==7){
        // std::cout<<"write.size"<<write_buffer_.size()<<std::endl;
        // }
    ...
    if (trans.is_write) {
        ...
        if (pending_rd_q_.count(trans.addr) == 1) {
            if (is_unified_queue_) {
                ...
                //std::cout<<"read_queue_"<<&read_queue_[0]<<std::endl;//RHY add
                //std::cout<<"read_queue_size"<<read_queue_.size()<<std::endl;//RHY add
                //std::cout<<"push_back"<<std::endl;//RHY add
            }
        }
        return true;
    }
...
void Controller::ScheduleTransaction() {
    ...
    for (auto it = queue.begin(); it != queue.end(); it++) {
        ...
        if (cmd_queue_.WillAcceptCommand(cmd.Rank(), cmd.Bankgroup(),
            ...
            if (!is_unified_queue_ && cmd.IsWrite()) {
                ...
                if (pending_rd_q_.count(it->addr) > 0) {
                    write_draining_ = 0;
        // if(channel_id_==7 && it->is_write){
        //    std::cout<<"R_W dep"<<",addr:"<<it->addr<<std::endl;
        //    }
        ...
        // if(channel_id_==7 && it->is_write){
        //    std::cout<<"it.addr:"<<it->addr<<" write_drain:"<<write_draining_<<" cycle:"<<clk_;
