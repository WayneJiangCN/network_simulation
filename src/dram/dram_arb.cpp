...
void DramArb::initializeBasicState() {
  ...
  for (int bank = 0; bank < num_banks; bank++) {
    down_arb_rd[bank] = false;
    down_arb_wr[bank] = false;
  }
}
    ...
    for (int upstream_id = 0; upstream_id < num_upstreams; upstream_id++) {
      ...
      if (!retryResp[bank][upstream_id]) {
        ...
        if (!responseQueue[bank].empty()) {
          ...
          // 尝试发送响应
          bool success =
              responsePorts[bank][target_upstream].sendTimingResp(pkt);
          ...
          if (success) {
            ...
            // 发送失败，设置重试标志
            D_DEBUG("DRAM_ARB", "响应发送失败: bank=%d, upstream=%d", bank,
                    upstream_id);
...
void DramArb::arbitrate() {
  ...
  for (int bank = 0; bank < num_banks; bank++) {
    ...
    if ((!down_arb_rd[bank] || !down_arb_wr[bank])) {
      for (int up = 0; up < num_upstreams; up++) {
...
bool DramArb::arbitrateReadRequests(int bank) {
  ...
  if (serving_upstream >= 0 && !readInBufs[bank][serving_upstream].empty()) {
    ...
    if (requestPorts[bank].sendTimingReq(pkt)) {
      ...
      down_arb_rd[bank] = true;
      down_arb_wr[bank] = true;
...
void DramArb::handleReqRetry(int bank_id) {
  assert(bank_id >= 0 && bank_id < num_banks);
  down_arb_rd[bank_id] = false;
  down_arb_wr[bank_id] = false;
