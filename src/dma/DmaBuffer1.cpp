...
//   DmaBuffer::DmaBuffer(const std::string &name, addr_t base_addr, int burst_num, int active_banks, uint32_t total_slice_num_cfg, uint32_t total_inst_num_cfg, const std::string& data_file_base_path, const std::string& data_file_suffix)
//       : SimObject(name), FileReader(total_slice_num_cfg, total_inst_num_cfg), base_addr_(base_addr), burst_num_(burst_num),
...
//     {
//       // 没有可读数据，拒绝请求
//       D_WARN("DmaBuffer", "[DMA] recvTimingReq,没有可读数据，拒绝请求");
...
//     D_INFO("DmaBuffer", "[DMA] recvTimingReq accepted for bank %d, stored pkt addr: 0x%x", 
//            bank_id, pkt->getAddr());
...
//            D_INFO("DMA", "Starting command %lu: dram_base_addr=%#d, lines=%d, inst_cnt  be %d, dram_final_addr=%#d",
//                   current_cmd_.cmd_id, controller.dram_base_addr[write_idx], current_cmd_.total_lines, inst_cnt, controller.dram_final_addr[write_idx]);
//            // 记录该buf对应的命令ID和回调
...
//       {
//         inst_cnt -= 1;
//         D_INFO("DMA", "inst_cnt decreased to %d", inst_cnt);
//         CompleteCommand();
...
//   {
//     // D_INFO("DECODER", "if_name: %s, checking against: %s", if_name.c_str(), computePorts[idx].name().c_str());
